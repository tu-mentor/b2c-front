...
const questions: Question[] = [
  { id: 'entrepreneurship', text: '¿Deseas emprender?', icon: HelpCircle },
  { id: 'funding', text: '¿Necesitas financiamiento para tu idea de negocio?', icon: HelpCircle },
  { id: 'adult', text: '¿Eres mayor de edad?', icon: HelpCircle },
];
...
export function ChildQuestions3({ children, onComplete, initialAnswers }: ChildQuestionsProps) {
  ...
  const handleSubmit = async () => {
    const allQuestionsAnswered = children.every((child) => 
      Object.keys(answers[child.id] || {}).length === questions.length
    );
    ...
    if (allQuestionsAnswered) {
      ...
      for (const child of children) {
        ...
        await childService.updateChildKit(child.id, {
          startBusiness: childAnswers['entrepreneurship'] === '1',
          financing: childAnswers['funding'] === '1',
          adult: childAnswers['adult'] === '1',
        });
      }
  ...
  return (
    <Card className="w-full max-w-3xl mx-auto mt-4">
      <CardHeader className="pb-2">
        <CardTitle className="text-xl font-bold text-center">Evaluación de Emprendimiento</CardTitle>
      </CardHeader>
      <CardContent>
        <motion.p
        ...
        >
          Por favor contesta las siguientes preguntas sobre emprendimiento para continuar.
        </motion.p>
        <AnimatePresence>
          {children.map((child) => (
            <motion.div
